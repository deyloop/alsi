...
void Assembler::process( const char* filename, const char* exefile) {
  ...
  std::map<std::string,labeltable> labels;
    ...
    if (segment == codeseg){
      ...
      if(word =="regSP"	) code.push_back(regSP	);
        ...
        switch (num_type) {
          ...
          case d:{
            double num = std::stod(word.c_str());
